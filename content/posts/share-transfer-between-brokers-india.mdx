...
The "Indian-friendly" platforms that exist (INDmoney, the original Vested platform, Stockal-now-Borderless) all work by opening you an account with a US partner broker — DriveWealth and Alpaca Securities being the two most common. INDmoney partners with both Alpaca and DriveWealth; Vested with DriveWealth. Until very recently, none of these platforms supported inbound ACATS for an existing RSU position — you could open a fresh account and buy new shares, but you couldn't bring your existing vested RSUs home. That's now changed: INDmoney supports inbound transfers via its Alpaca partnership, and so does Rovia.
...
**India-aware platforms layering on top of Alpaca.** Several platforms partner with Alpaca to offer Indian residents brokerage accounts. The newest of these is [Rovia](https://rovia.one), which we co-founded and which Vested is the editorial publication of. Rovia opens an Alpaca account for you, accepts inbound ACATS from your employer's US broker, and provides Indian-format reporting on top — capital gains in INR using SBI TT rates, Schedule FA-ready disclosures, lot-level visibility, and Form 67-ready dividend tracking.

We're not the only ones doing this. INDmoney has been around longer and serves a broader retail US-investing audience &mdash; it also supports inbound share transfer via Alpaca, ITR-format tax reports with lot-level capital gains and dividend breakdowns, and a wider asset universe (including OTC stocks and most US small/mid-caps). The original Vested platform serves the simpler retail use case but currently doesn't support inbound ACATS. The differences between INDmoney and Rovia come down to focus and pricing: Rovia is built specifically for RSU and equity-comp holders rather than general retail, brokerage is 0.15% versus 0.25% at INDmoney and Vested, and the lot-selection automation and harvest-suggestion workflow is geared at the cross-border tax optimization that matters when you're holding meaningful US equity through your employer.
